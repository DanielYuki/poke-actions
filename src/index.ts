  ...
  try {
    ...
    if (context.eventName === "issues") {
      ...
      if (action === "opened") {
        ...
        core.info(
          `Posted opening comment on issue #${issueNumber} (found by @${username})`
        );
        ...
        core.info(
          `Issue #${issueNumber} closed - @${username} caught ${pokemon.name}!`
        );
      }
    }
    ...
    // Handle pull request events
      ...
      if (action === "opened") {
        ...
        core.info(
          `Posted opening comment on PR #${prNumber} (opened by @${username})`
        );
      } else if (action === "closed") {
        ...
        if (pr.merged === true) {
          ...
          core.info(
            `PR #${prNumber} merged - @${author} caught ${pokemon.name}! (merged by @${merger})`
          );
        } else {
          await octokit.rest.issues.createComment({
            ...repo,
          ...
          core.info(
            `PR #${prNumber} closed without merge - The wild pokémon fled`
          );
        }
      }
    }
